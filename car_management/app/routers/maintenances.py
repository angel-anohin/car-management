...
def create_maintenance(maintenance: CreateMaintenanceDTO, db: Session = Depends(get_db)):
    db_car = MaintenanceRepository.get_car_by_id(db=db, carId=maintenance.carId)  # Проверка дали колата съществува
    db_garage = MaintenanceRepository.get_garage_by_id(db=db, garageId=maintenance.garageId)  # Проверка дали гаражът съществува
